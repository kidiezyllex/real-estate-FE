...
interface HomeContractUpdateFormProps {
  contractId: string;
}

const HomeContractUpdateForm = ({ contractId }: HomeContractUpdateFormProps) => {
  ...
  const { data: contractData, isLoading: isLoadingContract } = useGetHomeContractDetail({
    id: contractId
  });
  
  const { mutate: updateContractMutation, isPending } = useUpdateHomeContract();
    ...
    updateContractMutation({
      ...
      onSuccess: (data) => {
        if (data.statusCode === 200) {
          ...
          router.push(`/admin/contracts/home-contracts/${contractId}`);
        } else {
  ...
  if (!contract) {
    return (
      <div className="space-y-8 bg-mainCardV1 p-6 rounded-lg border border-lightBorderV1">
        <p className="text-mainDangerV1">Không tìm thấy thông tin hợp đồng thuê nhà</p>
      </div>
    );
  }
      ...
      >
        <Card className="border border-lightBorderV1 bg-mainBackgroundV1">
          <CardContent className="pt-6">
            ...
            <div className="mb-6 bg-blue-50 p-4 rounded-md border border-blue-200">
              <p className="text-sm text-blue-700">
                ...
                <br />
                {contract.contractCode && <>Số hợp đồng: <span className="font-semibold">{contract.contractCode}</span></>}
                <br />
                ...
                <br />
                {typeof contract.guestId === 'object' && 'fullname' in contract.guestId ? (
                  <>Khách hàng: <span className="font-semibold">{contract.guestId.fullname}</span></>
                ) : (
                  <>Mã khách hàng: <span className="font-semibold">{contract.guestId}</span></>
                )}
              </p>
            </div>
          ...
          <CardFooter className="flex justify-end space-x-4 border-t border-lightBorderV1 pt-6">
            ...
            <Button
              type="submit"
              form="home-contract-update-form"
              disabled={isPending}
...
export default HomeContractUpdateForm; 