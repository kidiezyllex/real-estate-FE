...
import { ServiceContractDeleteDialog } from "@/components/ServiceContractsPage/ServiceContractDeleteDialog";
...
export default function ServiceContractDetails({ contractId }: ServiceContractDetailsProps) {
  const router = useRouter();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  const { data: contractData, isLoading, error } = useGetServiceContractDetail({
    id: contractId
  });
  ...
  useEffect(() => {
    if (error) {
      toast.error("Không thể tải thông tin hợp đồng dịch vụ");
      router.push("/admin/contracts/service-contracts");
    }
  }, [error, router]);

  const handleEdit = () => {
    router.push(`/admin/contracts/service-contracts/${contractId}/edit`);
  };
      ...
      >
        ...
        <div className="flex flex-col gap-4 mt-6">
          ...
          <Card className="shadow-sm border border-lightBorderV1">
            {contract && (
              <ServiceContractDetailInfo contract={contract} />
            )}
          </Card>
        </div>
      </motion.div>
