...
import { useGetHomeContracts, useDeleteHomeContract, useSearchHomeContracts } from "@/hooks/useHomeContract";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
export default function HomeContractsPage() {
  ...
  const { data: contractsData, isLoading, refetch } = useGetHomeContracts();
  const { data: searchResults, isLoading: isSearching } = useSearchHomeContracts({
  ...
  const filteredAndSortedContracts = () => {
    ...
    result.sort((a, b) => {
      if (sortBy === "newest") {
        ...
        return sortDirection === 'asc' ? (a as any).duration - (b as any).duration : (b as any).duration - (a as any).duration;
      }
      return 0;
    });
    ...
    <div className="space-y-8 bg-mainBackgroundV1 p-6 rounded-lg border border-lightBorderV1">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage>Hợp đồng thuê nhà</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      <motion.div
